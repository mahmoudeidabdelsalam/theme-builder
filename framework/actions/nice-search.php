...
function ajax_listings() {
  ...
  (
    ...
    foreach( $results as $result )
      $titles[] = ['value' => addslashes( $result->post_title ), 'content' => wp_trim_words( $result->post_content, '40', '...' ), 'posttype' => get_post_type($result->ID), 'url' => get_permalink($result->ID)];
      if(count($titles) == 0 ){
        $titles[] = ['value' => 'No matching search results - Please change your search terms', 'content' => '', 'post' =>'', 'url' => '#'];
      }
